...
def compare_trajectory(original_trajectory, modified_trajectory, title, points=None,visualize_workspace=False, workspace_type=None, workspace_bounds=None, elev=30, azim=45, file_name=None):
    ...
    if visualize_workspace:
        if workspace_type is None or workspace_type=="general": 
            ...
            # Visualize the arm's spherical shell workspace (between r_min and r_max)
            if workspace_bounds is not None:
                center = workspace_bounds.get("centre", [0, 0, 0])
                r_max = workspace_bounds.get("r_max", 1.0)
                r_min = workspace_bounds.get("r_min", 0.0)
                # Create a mesh for the outer sphere (r_max)
                u = np.linspace(0, 2 * np.pi, 100)
                v = np.linspace(0, np.pi, 100)
                x_outer = center[0] + r_max * np.outer(np.cos(u), np.sin(v))
                y_outer = center[1] + r_max * np.outer(np.sin(u), np.sin(v))
                z_outer = center[2] + r_max * np.outer(np.ones_like(u), np.cos(v))
                ax1.plot_surface(x_outer, y_outer, z_outer, color='green', alpha=0.10, edgecolor='none')
                # Create a mesh for the inner sphere (r_min)
                if r_min > 0:
                    x_inner = center[0] + r_min * np.outer(np.cos(u), np.sin(v))
                    y_inner = center[1] + r_min * np.outer(np.sin(u), np.sin(v))
                    z_inner = center[2] + r_min * np.outer(np.ones_like(u), np.cos(v))
                    ax1.plot_surface(x_inner, y_inner, z_inner, color='white', alpha=0.10, edgecolor='none')
                ax1.text(center[0], center[1], center[2] + r_max + 0.05, "Arm Workspace", color='green', fontsize=14, ha='center')
            # if workspace_bounds is not None:
            #     center = workspace_bounds.get("centre", [0, 0, 0])
