...
import copy
if "original_trajectory" not in st.session_state:import streamlit as st
import json
import os
import numpy as np
import plotly.graph_objects as go
...
if 'zero_shot_trajectory' not in st.session_state:
    st.session_state.zero_shot_trajectory = {}
if 'final_trajectory' not in st.session_state:
    st.session_state.final_trajectory = {}
...
uploaded_file = st.file_uploader("Upload trajectory JSON file", type=["json"])
...
if uploaded_file:
    ...
    with col1:
        fig_traj=go.Figure()
        plot_trajectory(fig_traj,st.session_state.original_trajectory, st.session_state.modified_trajectory,detect_objects(),instruction)
        st.plotly_chart(fig_traj,use_container_width=True)
        st.markdown("<br>", unsafe_allow_html=True)  # Add space
    with col2:
        fig_vel=go.Figure()
        plot_velocity(fig_vel,st.session_state.original_trajectory, st.session_state.modified_trajectory)
        ...
        if run_adaptation:
            ...
            # Get the feedback type 
                ...
                if st.session_state.feedback.lower() == "yes":
                        ...
                        if len(st.session_state.global_feedbacks) !=0 or len(local_feedbacks)!=0: # there are feedbacks involved after zero shot adaptation
...
def detect_objects():
    ...
    for item in objs:
        ...
        if 'dimensions' not in item.keys():
            item.update({'dimensions': [config.DEFAULT_DIMENSION]*3})
    return objs
def plot_trajectory(original, modified, objects,instruction):
    st.subheader(f"Instruction: {instruction}")
    fig = go.Figure()
...
def plot_velocity(original, modified):
    fig = go.Figure()
    ...
    fig.update_layout(title='Velocity Profile', xaxis_title='Time Step', yaxis_title='Velocity')
    st.plotly_chart(fig)

st.title('Trajectory Adaptation with LLM')
...
if uploaded_file:
    ...
    llm_choice = st.selectbox("Select LLM:", ["openai", "claude", "gemini"])
    robot_type = st.selectbox("Select Robot Type:", ["Drone", "Arm", "GroundRobot", "None"])
    config = Config()
    config.api_name = llm_choice
    agent = LLM_Agent(config)
    ...
    try:
        # Request feedback
        feedback_type=st.selectbox("Feedback Type:", ["original","current"], index=None, placeholder="Select feedback type")
        feedback = st.text_input("Provide Feedback (or type YES if satisfied):")
        if st.button("Run Adaptation"):
            # Get the feedback type 
                if feedback=="":
                    st.session_state.new_instruction = instruction
                else:
                    ...
                    if feedback_type == "original":
                        ...
                        get_trajectory_param = True
                        global_feedbacks.append(feedback)
                        st.session_state.new_instruction = instruction + " ".join(global_feedbacks)
                    elif feedback_type == "current":
                        get_trajectory_param = False  
                        local_feedbacks.append(feedback)
                        st.session_state.new_instruction = feedback
                ...
                if feedback is None or feedback_type=="original":
                    ...
                    print("current context, trajectory extracted from modified list")# Request feedback
                    st.session_state.original_trajectory=modified_trajectory_list[-1]
                ...
                modified_trajectory_list.append(st.session_state.modified_trajectory)
                # if st.session_state.feedback:
                if feedback.lower() == "yes":
                        # Save the trajectory and results
                        get_trajectory_param = True
