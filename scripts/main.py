...
if __name__=="__main__":
    ...
    parser.add_argument(
        "--workspace_bound",
        action="store_true",
        help="Visualize the reachable boundary"
    )
    parser.add_argument(
        "--sim",
        action="store_true",
        help="Activate the simulation if this flag is set."
    )
    ...
    if args.robot_type=='Drone':
        workspace_type="cuboidal" if args.workspace_bound else "general"
        workspace_bounds = config.workspace_bounds_drone
    elif args.robot_type=='Arm':
        workspace_type="arm" if args.workspace_bound else "general"
    ...
    try:
        while True:
            ...
            modified_trajectory = convert_from_vector(constraint_satisfaction_module(
                ...
                apply_spline_smooth=config.apply_spline_smooth
            ))
            ...
            compare_trajectory(
                ...
                title=instruction,
                visualize_workspace=config.visualise_boundary,
                workspace_type=workspace_type,
                workspace_bounds=workspace_bounds
            )
    ...
    if args.sim:
        from OVITA.waypoint_executer_arm import KUKASIM
        with open(cwd+"/"+save_dir+"/" +args.llm+"/"+ file_name, 'r') as file:    
            data = json.load(file)
        sim_obj=KUKASIM(data)
        sim_obj._exec()
