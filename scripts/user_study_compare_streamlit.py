...
import json
from OVITA.util import convert_to_vector,resample_trajectory
import copy
...
if "data" not in st.session_state:
    with open("user_distribution_comparison.json","r") as file: 
...
def save_response():
    ...
    with open(f"results_user_study/latte_vs_ovita/{user_name}_response.json", "w") as file: 
...
def get_trajectory(option):
    ...
    try:
        ...
        latte_file_path=copy.deepcopy(file_path).replace("/results_ovita/","/results_latte/")
        if not file_path:
        ...
        with open(latte_file_path, "r") as infile_1:
            latte_data_point = json.loads(infile_1.read())
        # modefied_latte_trajectory=latte_data_point["zero_shot_trajectory"]["modified trajectory"]
        ...
        latte_details=latte_data_point.get('zero_shot_trajectory', {})
        ...
        modefied_latte_trajectory=resample_trajectory(convert_to_vector(latte_details.get('modified trajectory', "No modified trajectory available.")),100)
        ...
        return (
            (original_traj, zero_shot_traj, final_traj, modefied_latte_trajectory,objects),
...
(original_traj, zero_shot_traj, final_traj, modefied_latte_trajectory,objects),(instruction, global_feedbacks, local_feedbacks, zero_interpret, final_interpret,LLM)=get_trajectory(selected_trajectory)
# Getting the data
...
if selected_trajectory is not None :
    col1, col2 = st.columns(2)
    fig1,vel1=compare_trajectory(original_traj,modefied_latte_trajectory,"Trajectory 1",objects)
    if final_traj is not None: 
        fig2,vel2=compare_trajectory(original_traj,final_traj,"Trajectory 2",objects)
    else:
        fig2,vel2=compare_trajectory(original_traj,zero_shot_traj,"Trajectory 2",objects)  
    with col1:
        st.plotly_chart(fig1, use_container_width=True)
    with col2:
        st.plotly_chart(vel1, use_container_width=True,key='speed profile')

    rating_zero_shot = st.slider(
        "Rating (1-5) Trajectory 1",
        min_value=1, 
        max_value=5, 
    ...
    with col2:
        st.plotly_chart(vel2, use_container_width=True,key='OVITA speed profile')
        
   
    rating_final = st.slider(
        "Rating (1-5) Trajectory 2:",
        min_value=1, 
        max_value=5, 
        value=3,  # Default value
        step=1
...
if visualized_button:
    if selected_trajectory is None:
        ...
        remaining_trajectories = [t for t in trajectory_options if t not in visualized_trajectories]
        response_dict={
            "path": st.session_state.paths[selected_trajectory], "Latte": rating_zero_shot, "OVITA": rating_final if final_traj is not None else rating_zero_shot,"LATTE_vs_OVITA":rating_code_interpret
        }
